...
node {
  ...
  stage('SonarQube analysis') {
	...
	// requires SonarQube Scanner 2.8+
		...
		sh "${scannerHome}/bin/sonar-scanner -Dsonar.java.binaries=build/core/** -Dsonar.projectKey=jmeter-test-2 -Dsonar.projectName=jmeter-test-2 -Dsonar.projectVersion=3.3.1 -Dsonar.sources=. -Dsonar.organization=elliot_jds-bitbucket -Dsonar.branch.name=topic_branchscan"
	}
